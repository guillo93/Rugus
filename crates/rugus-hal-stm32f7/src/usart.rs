//! USART2 para STM32F7 — PA2 TX / PA3 RX, polled, 8N1.
//!
//! Driver de consola bloqueante por acceso MMIO directo, gemelo del de F4 pero
//! con el mapa de registros de la IP USART de la familia F7 (CR1/CR2/CR3/BRR al
//! principio, e **ISR/RDR/TDR** en lugar del SR/DR único de F4). USART2 cuelga
//! de APB1.
//!
//! Incluye el mismo autotest de lazo cerrado por **half-duplex single-wire**
...
impl Usart2 {
    ...
    pub fn write_byte(&mut self, b: u8) {
        // SAFETY: registros MMIO de USART2; espera a TDR vacío antes de escribir.
        unsafe {
            while read_reg(ISR) & ISR_TXE == 0 {}
            write_reg(TDR, b as u32);
        }
    }
    ...
    pub fn try_read_byte(&mut self) -> Option<u8> {
        ...
        unsafe {
            if read_reg(ISR) & ISR_RXNE != 0 {
                Some((read_reg(RDR) & 0xFF) as u8)
            } else {
                None
            }
        }
...
///
/// Pensada para el `#[interrupt] fn USART2`: lee `RDR` si `RXNE` está alto (lo
/// que también limpia el flag y desactiva la pendiente de la IRQ). Devuelve
/// `None` si la IRQ se disparó por otra causa. La base de USART2 es fija, así
/// que no necesita el handle propietario.
pub fn isr_read_byte() -> Option<u8> {
    // SAFETY: registros MMIO de USART2; leer RDR limpia RXNE atómicamente.
    unsafe {
        if read_reg(ISR) & ISR_RXNE != 0 {
            Some((read_reg(RDR) & 0xFF) as u8)
        } else {
