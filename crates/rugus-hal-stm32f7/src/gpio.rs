...
//! familia F7 tiene layout idéntico al de la F4 (base `0x4002_0000`, stride
//! `0x400`). Los LEDs de la STM32F769I-DISCO (UM2033 §6.5) son un caso
//! particular construido sobre [`Pin`]; a diferencia de la F4 reparten en
//! varios puertos (A/D/J).

use crate::pac;
use core::ptr::{read_volatile, write_volatile};
use rugus_hal::GpioPin;

/// Base del primer puerto GPIO (GPIOA) en la familia STM32F7.
...
/// User LEDs en STM32F769I-DISCO (repartidos en varios puertos, activos en alto).
#[derive(Clone, Copy, Debug)]
pub enum DiscoLed {
    /// LD1, rojo, PJ13.
    Red,
...
impl DiscoLed {
    #[inline]
    fn location(self) -> (Port, u8) {
        match self {
            DiscoLed::Red => (Port::J, 13),
            DiscoLed::Green => (Port::J, 5),
            DiscoLed::Red2 => (Port::A, 12),
            DiscoLed::Green2 => (Port::D, 4),
        }
    }
}
...
impl LedPin {
    ...
    pub fn new(_rcc: &pac::RCC, led: DiscoLed) -> Self {
        let (port, pin) = led.location();
        Self {
            pin: Pin::new(port, pin, PinConfig::output()),
        }
    }
}
