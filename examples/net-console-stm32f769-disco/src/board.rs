//! Operaciones de placa del tier full para STM32F769I-DISCO.
//!
//! Aporta las piezas de [`BoardOps`] que dependen del silicio F7 —el GPIO
...
//!
//! El puerto se codifica `0=A … 10=K` y el pin `0..=15`, igual que el mapa de
//! `Port` del HAL F7. El operador está autenticado (canal gateado): manipular
//! pines de RMII/QSPI puede tumbar el enlace, es su responsabilidad.

use rugus_core::syscall::lite::GpioLevel;
use rugus_core::Errno;
use rugus_hal::GpioPin;
use rugus_hal_stm32f7::gpio::{Pin, PinConfig, Port, Pull};
use rugus_personality_full::BoardOps;

/// Identidad de la placa para `cosmos`/`ecosystem`.
const BOARD_NAME: &str = "f769-disco";

/// Construye las operaciones de placa del F769: GPIO real + stubs `Enosys` para
...
/// Traduce el índice de puerto (`0=A … 10=K`) al enum del HAL.
