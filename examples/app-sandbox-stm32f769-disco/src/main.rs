...
/// Identidad para IDENTIFY/ENQ: tier full sobre silicio F769.
const TIER: &str = "full";
...
fn main() -> ! {
    ...
    unsafe {
        ...
        psk::install(Qspi::new(dp.QUADSPI, &dp.RCC).expect("qspi init"));
        AUTH_HOOKS = Some(auth::hooks());
        defmt::info!("PSK store (QSPI): provisioned={=bool}", psk::provisioned());
