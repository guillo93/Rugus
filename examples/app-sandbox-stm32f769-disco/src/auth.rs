//! Cableado de la autenticación de canal (F6.3b) para la consola de red F769.
//!
...
//! construyen esos ganchos para la F769-DISCO, idénticos en espíritu a los de la
//! personalidad lite (F103): la PSK vive en un almacén que la consola no puede
//! leer (subsector QSPI dedicado, [`crate::psk`]), el HMAC se recalcula con
//! `rugus-crypto` y se compara en tiempo constante, y el nonce sale de un CSPRNG
//! software sembrado con el contador de ciclos DWT + uptime (la F7 no usa TRNG
//! por hardware en este build).
...
/// Recalcula `HMAC-SHA256(PSK, nonce)` con la PSK en QSPI y lo compara en tiempo
/// constante con `proof`. `rush` nunca ve la PSK: solo este módulo la lee, y
/// solo para alimentar el HMAC. Devuelve `false` si no hay PSK.
