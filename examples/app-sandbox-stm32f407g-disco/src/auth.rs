//! Cableado de la autenticación de canal (F6.4c) para la consola UART F407.
//!
...
//! construyen esos ganchos para la F407G-DISC1, gemelos de los del F769 y la
//! personalidad lite (F103): la PSK vive en un almacén que la consola no puede
//! leer (sector de flash interna dedicado, [`crate::psk`]), el HMAC se recalcula
//! con `rugus-crypto` y se compara en tiempo constante, y el nonce sale de un
//! CSPRNG software sembrado con el contador de ciclos DWT + uptime.
...
/// Recalcula `HMAC-SHA256(PSK, nonce)` con la PSK de la ventana de flash y lo
/// compara en tiempo constante con `proof`. `rush` nunca ve la PSK: solo este
/// módulo la lee, y solo para alimentar el HMAC. Devuelve `false` si no hay PSK.
